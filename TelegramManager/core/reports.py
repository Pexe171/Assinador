# Caminho: TelegramManager/core/reports.py
...
from TelegramManager.core.automation import AutomationEngine, AutomationTask, TaskStatus
...
@dataclass(frozen=True)
class AdditionSummary:
    """Consolida estatísticas básicas do módulo de adição."""
...
class ReportService:
    ...
    def __init__(
        ...
        *,
        ...
        self._addition_manager = addition_manager
    ...
    def gerar_resumo_automacao(self) -> AutomationSummary:
        ...
        total = len(tarefas)
        agendadas = sum(1 for tarefa in tarefas if tarefa.status == TaskStatus.SCHEDULED)
        em_andamento = sum(1 for tarefa in tarefas if tarefa.status == TaskStatus.RUNNING)
        concluidas = sum(1 for tarefa in tarefas if tarefa.status == TaskStatus.COMPLETED)
        falhas = sum(1 for tarefa in tarefas if tarefa.status == TaskStatus.FAILED)
        progresso_medio = (
    ...
    def gerar_resumo_adicoes(self) -> AdditionSummary:
        """Calcula indicadores principais das tarefas de adição."""
        with self._addition_manager._db.session() as session:
            stmt = select(
                func.count(AdditionJob.id),
                func.sum(AdditionJob.success_count),
                func.sum(AdditionJob.fail_count),
            )
            total_jobs, total_add, total_fail = session.execute(stmt).first()

            total_jobs = total_jobs or 0
            total_add = total_add or 0
            total_fail = total_fail or 0
            total_ops = total_add + total_fail
            taxa_sucesso = (total_add / total_ops) * 100 if total_ops > 0 else 0.0

            return AdditionSummary(
                total_jobs=int(total_jobs),
                total_adicionados=int(total_add),
                total_falhas=int(total_fail),
    ...
    def gerar_atividade_diaria(self) -> List[DailyAutomationActivity]:
        ...
        for tarefa in tarefas:
            if tarefa.status != TaskStatus.COMPLETED:
                continue
    ...
    def _proxima_execucao(tarefas: Iterable[AutomationTask]) -> datetime | None:
        proximas = [
            tarefa.agendamento
            for tarefa in tarefas
            if tarefa.status == TaskStatus.SCHEDULED
        ]
